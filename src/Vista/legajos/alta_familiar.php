...
if (!empty($empleado->cuit)){
    if($permisos['bloque_grupo_familiar']){
			...
			switch ($campo) {
				...
				case 'desgrava_afip':
					if($permiso) {
						...
						$vars_template[$input][0]['SPAN_DESGRAVA_AFIP'][0]['VALUE'] = ($familiar->desgrava_afip) ? $parametricos['porcentaje_desgrava'][$familiar->desgrava_afip]['nombre'] : '';
				...
				case 'fecha_hasta':
					if($permiso) {
						$vars_template[$input][0]['C_FECHA_HASTA'][0]['FECHA_HASTA'] = is_object($familiar->fecha_hasta) ? $familiar->fecha_hasta->format('d/m/Y'):'';
					} else {
						$vars_template[$input][0]['SPAN_FECHA_HASTA'][0]['VALUE'] = is_object($familiar->fecha_hasta) ? $familiar->fecha_hasta->format('d/m/Y'):'';
					}
					break;
				...
				case 'tipo_discapacidad':
					if($permiso) {
						...
						$vars_template[$input][0]['SPAN_TDISCAPACIDAD'][0]['VALUE'] = $parametricos['tipo_discapacidad'][$familiar->fam_discapacidad->id_tipo_discapacidad]['nombre'];
		...
		$vars_template['CAMPOS_FAMILIAR'][0]	+= [
			'FORM'		=> \App\Helper\Vista::get_url("index.php/legajos/alta_familiar/{$empleado->cuit}"),
			'BOTON_SUBMIT' 	=> 'GUARDAR',
	    ...
	    $vars_template['CAMPOS_FAMILIAR'][0]['TIPO_FORM'] = 'alta_familiar';
...
JS;
		...
		$vars_vista['SUBTITULO'] = 'Alta de Familiar';
