...
class FuncionEjecutiva extends Modelo {
/** @var int */
	public $id;
/** @var int */
...
/** @var string */
	...
	static public function obtener($id=null){
		...
		$sql	= <<<SQL
			SELECT {$campos}
			FROM convenio_funciones_ejecutivas
			WHERE id = :id
SQL;
		$cnx = new Conexiones();
...
SQL;
	...
	public function alta(){
		...
		$sql = 'INSERT INTO convenio_funciones_ejecutivas('.implode(',', $campos).') VALUES (:'.implode(',:', $campos).')';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);
		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'FuncionEjecutiva';
	...
	public function modificacion(){
		...
		$sql = 'UPDATE convenio_funciones_ejecutivas SET '.implode(',', $campos).' WHERE id = :id';
	...
	public function baja(){
		...
		$sql	= 'UPDATE convenio_funciones_ejecutivas SET borrado = 1 WHERE id = :id';
		$res	= $cnx->consulta(Conexiones::UPDATE, $sql, $sql_params);
	...
	public static function listadoFuncionesEjecutivas($params=array(), $count = false) {
		$cnx	= new Conexiones();
		$sql_params = [];
		$where = [];
		$condicion = '';
		...
		$sql= <<<SQL
			SELECT 
		    fe.id,
		    fe.nombre AS agrupamiento,
			mv.nombre AS modalidad_vinculacion,
			sr.nombre AS situacion_revista
SQL;

		$from = <<<SQL
			FROM convenio_funciones_ejecutivas fe
			INNER JOIN convenio_modalidad_vinculacion mv  ON mv.id =  fe.id_modalidad_vinculacion
			INNER JOIN convenio_situacion_revista sr  ON sr.id =  fe.id_situacion_revista
SQL;
...
SQL;
    ...
    $counter_query	= "SELECT COUNT(DISTINCT fe.id) AS total {$from}";
	...
	if(!empty($params['search'])){
		$indice = 0;
		$search[]	= <<<SQL
		(fe.nombre LIKE :search{$indice} OR 
...
SQL;
	...
	/*public function agrupamientos($param){
		$cnx = new Conexiones();
		$sql_params	= [
	...
	public function ajaxGetAgrupamientos($param){
		$cnx = new Conexiones();
		$sql_params	= [
