...
class PresupuestoSaf extends Modelo {
/** @var int */
	public $id;
/** @var int */
	public $codigo;
...
SQL;
	...
	public function alta(){
		...
		$sql = 'INSERT INTO presupuesto_saf(nombre, codigo) VALUES (:nombre, :codigo)';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);

		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'PresupuestoSaf';
	...
	public function modificacion(){
		...
		$sql	= 'UPDATE presupuesto_saf SET '.implode(',', $campos).' WHERE id = :id';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);
		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'PresupuestoSaf';
	...
	public function baja(){
		...
		$sql	= 'UPDATE presupuesto_saf SET borrado = 1 WHERE id = :id';
	...
	public static function listadoPresupuestoSaf($params=array(), $count = false) {
		$cnx	= new Conexiones();
		$sql_params = [];
		$where = [];
		$condicion = '';
	...
	public function validar() {
		...
		$reglas		= [
			...
			'nombre'	=> ['required','texto', 'min_length(5)', 'max_length(200)', 
							'saf_unico(:codigo,:id)' => function($input, $codigo, $id){
				...
				$sql = <<<SQL
							SELECT count(*) count FROM presupuesto_saf WHERE nombre = :nombre AND codigo = :codigo $where_id
SQL;
				$con = new Conexiones();
		...
		$validator->customErrors([
            'saf_unico'      => 'Ya existe una SAF con el código y nombre.',
        ]);
