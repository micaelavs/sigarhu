...
class Agrupamiento extends Modelo {
/** @var int */
	public $id;
/** @var int */
...
SQL;
	...
	public function alta(){
		...
		$sql = 'INSERT INTO convenio_agrupamientos('.implode(',', $campos).') VALUES (:'.implode(',:', $campos).')';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);
		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'Agrupamiento';
			Logger::event('alta_convenio_agrupamiento', $datos);
		}
		return $res;
	}
	...
	public function modificacion(){
		...
		$sql = 'UPDATE convenio_agrupamientos SET '.implode(',', $campos).' WHERE id = :id';
		$res = $cnx->consulta(Conexiones::UPDATE, $sql, $sql_params);
		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'Agrupamiento';
			Logger::event('modificacion_convenio_agrupamiento', $datos);
		}
		return $res;
	}
	...
	public function baja(){
		...
		$sql	= 'UPDATE convenio_agrupamientos SET borrado = 1 WHERE id = :id';
		$res	= $cnx->consulta(Conexiones::UPDATE, $sql, $sql_params);
		...
		if (!empty($res) && $res > 0) {
			...
			Logger::event('baja_convenio_agrupamiento', $datos);
		}
		return $flag;
	}

	public static function listadoAgrupamientos($params=array(), $count = false) {
		$cnx	= new Conexiones();
		$sql_params = [];
		$where = [];
		$condicion = '';
...
SQL;
 		$condicion =
 		<<<SQL
 			WHERE a.borrado = 0 and mv.borrado = 0 AND sr.borrado = 0
SQL;

    $counter_query	= "SELECT COUNT(DISTINCT a.id) AS total {$from}";
