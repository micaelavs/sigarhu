...
class PresupuestoActividad extends Modelo {
/** @var int */
	public $id;
/** @var int */
	public $codigo;
...
SQL;
	...
	public function alta(){
		...
		$sql = 'INSERT INTO presupuesto_actividades(nombre, codigo) VALUES (:nombre, :codigo)';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);

		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'PresupuestoActividad';
	...
	public function modificacion(){
		...
		$sql = 'UPDATE presupuesto_actividades SET '.implode(',', $campos).' WHERE id = :id';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);
		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'PresupuestoActividad';
	...
	public function baja(){
		...
		$sql	= 'UPDATE presupuesto_actividades SET borrado = 1 WHERE id = :id';
	...
	public static function listadoActividades($params=array(), $count = false) {
		$cnx	= new Conexiones();
		$sql_params = [];
		$where = [];
		$condicion = '';
	...
	public function validar() {
		...
		$reglas		= [
			...
			'nombre'	=> ['required','texto', 'min_length(5)', 'max_length(200)',
							'actividad_unica(:codigo,:id)' => function($input, $codigo, $id){
				...
				$sql = <<<SQL
							SELECT count(*) count FROM presupuesto_actividades WHERE nombre = :nombre AND codigo = :codigo $where_id
SQL;
				$con = new Conexiones();
		...
		$validator->customErrors([
            'actividad_unica'      => 'Ya existe una actividad con el código y nombre.',
        ]);
