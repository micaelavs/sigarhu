...
class PresupuestoJurisdiccion extends Modelo {
/** @var int */
	public $id;
/** @var int */
	public $codigo;
...
SQL;
	...
	public function alta(){
		...
		$sql = 'INSERT INTO presupuesto_jurisdicciones(nombre, codigo) VALUES (:nombre, :codigo)';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);

		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'PresupuestoJurisdiccion';
	...
	public function modificacion(){
		...
		$sql	= 'UPDATE presupuesto_jurisdicciones SET '.implode(',', $campos).' WHERE id = :id';
		$res = $cnx->consulta(Conexiones::INSERT, $sql, $sql_params);
		if($res !== false){
			$datos = (array) $this;
			$datos['modelo'] = 'PresupuestoJurisdiccion';
	...
	public function baja(){
		...
		$sql	= 'UPDATE presupuesto_jurisdicciones SET borrado = 1 WHERE id = :id';
	...
	public static function listadoPresupuestoJurisdiccion($params=array(), $count = false) {
		$cnx	= new Conexiones();
		$sql_params = [];
		$where = [];
		$condicion = '';
	...
	public function validar() {
		...
		$reglas		= [
			...
			'nombre'	=> ['required','texto', 'min_length(5)', 'max_length(200)', 
							'jurisdiccion_unica(:codigo,:id)' => function($input, $codigo, $id){
				...
				$sql = <<<SQL
							SELECT count(*) count FROM presupuesto_jurisdicciones WHERE nombre = :nombre AND codigo = :codigo $where_id
SQL;
				$con = new Conexiones();
		...
		$validator->customErrors([
            'jurisdiccion_unica'      => 'Ya existe una Jurisdicción con el código y nombre.',
        ]);
