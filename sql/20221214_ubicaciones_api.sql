...
DROP TRIGGER IF EXISTS `sigarhu_aud`.`ubicaciones_api_tg_insert`;

DELIMITER $$
USE `sigarhu_aud`$$
CREATE DEFINER=`fgarcia`@`%` TRIGGER `ubicaciones_api_tg_insert` AFTER INSERT ON `ubicaciones_api` FOR EACH ROW BEGIN
...
ALTER TABLE `sigarhu_aud`.`_registros_abm` 
...
DROP TRIGGER IF EXISTS `sigarhu`.`ubicaciones_api_alta`;

DELIMITER $$
USE `sigarhu`$$
CREATE DEFINER=`fgarcia`@`%` TRIGGER `ubicaciones_api_alta` AFTER INSERT ON `ubicaciones_api` FOR EACH ROW
BEGIN
...
DROP TRIGGER IF EXISTS `sigarhu`.`ubicaciones_api_tg_modificacion`;

DELIMITER $$
USE `sigarhu`$$
CREATE DEFINER=`fgarcia`@`%` TRIGGER `ubicaciones_api_tg_modificacion` AFTER UPDATE ON `ubicaciones_api` FOR EACH ROW
BEGIN
INSERT INTO sigarhu_aud.ubicaciones_api (
	`id_usuario`,
	`fecha_operacion`,
